...
export const TemplateElegant = ({ freelancer, client, works, logo, invoiceNumber, notes }) => {
    ...
    return (
        <div className="max-w-4xl mx-auto bg-white p-6 sm:p-8 font-sans text-sm">
            {/* Header with logo and company details */}
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-lg sm:text-xl font-semibold text-gray-800">
                            {freelancer.name || 'Studio Shoowe'}
                        </h1>
                        <p className="text-gray-600 text-xs sm:text-sm">
                            {freelancer.role || 'Creative Studio'}
                        </p>
                    </div>
                </div>

                <div className="text-right text-gray-600 text-xs sm:text-sm">
                    <p>{freelancer.location.city || '123 Anywhere St.,'}</p>
                    <p>{freelancer.location.country || 'Any City, ST 12345'}</p>
                </div>
            </div>
            ...
            {/* Billing details */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8 text-xs sm:text-sm">
                <div>
                    <h3 className="font-semibold text-gray-800 mb-2">PAYABLE TO</h3>
                    <p className="text-gray-600">{client.name || 'Mrs. Helena Paquet'}</p>
                    <p className="text-gray-600">{client.phone || '123 Anywhere St., Any City'}</p>
                    <p className="text-gray-600">{client.email || '123 Anywhere St., Any City'}</p>
                </div>
                <div>
                    <h3 className="font-semibold text-gray-800 mb-2">BANK DETAILS</h3>
                    <p className="text-gray-600">{client.company || 'Salford & Co.'}</p>
            ...
            <div className="mb-8 overflow-x-auto">
                <div className="min-w-[500px]">
                    {/* Table header */}
                    <div className="bg-teal-500 text-white text-xs sm:text-sm font-semibold">
                        <div className="grid grid-cols-4 gap-2 sm:gap-4 p-3 sm:p-4">
                            <div>ITEM DESCRIPTION</div>
                            <div className="text-center">QTY</div>
                            <div className="text-center">PRICE</div>
                            <div className="text-right">TOTAL</div>
                    ...
                    <div className="bg-white">
                        {works.map((work, idx) => (
                            <div
                                key={idx}
                                className="grid grid-cols-4 gap-2 sm:gap-4 p-3 sm:p-4 border-b border-gray-100 text-gray-700 text-xs sm:text-sm"
                            >
                                <div>{work.fieldOfWork}</div>
                                <div className="text-center">{work.quantity || '1'}</div>
                                <div className="text-center">₹{work.cost.toFixed(2)}</div>
                                <div className="text-right">
                                    ₹{(work.cost * (work.quantity || 1)).toFixed(2)}
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="border-t border-gray-200 pt-6 mt-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs sm:text-sm text-gray-600 gap-3 sm:gap-0">
                    <div className="flex items-center gap-2">
                        <div className="w-4 h-4 bg-teal-500 rounded-full flex items-center justify-center">
                            <span className="text-white text-xs">🌐</span>
                        </div>
