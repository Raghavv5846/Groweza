...
export const TemplateColorful = ({
    ...
    const subtotal = works.reduce((sum, w) => sum + w.cost, 0);
    const packageDiscount = subtotal * 0.3; // 30% package discount
    const total = subtotal - packageDiscount;

    return (
        <div className="max-w-3xl mx-auto bg-white p-6 sm:p-8 font-sans text-sm text-gray-800">
            {/* Header with logo and freelancer info */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-6">
                <div className="flex items-center">
                    {/* Modern geometric logo */}
                    {freelancer.invoiceLogo ? (
                        <img
                            src={freelancer.invoiceLogo}
                            alt="Freelancer Logo"
                            className="w-16 h-16 object-contain mr-4 rounded-full border border-gray-200"
                        />
                    ) : (
                    ...
                    <div>
                        <h1 className="text-xl sm:text-2xl font-bold tracking-wide text-gray-800 mb-1">
                            {freelancer.name || 'SAMIRA HADID'}
                        </h1>
                        <p className="text-xs sm:text-sm text-gray-500 tracking-widest uppercase">
                            {freelancer.role || 'ARCHITECT'}
                        </p>
                        <p className="text-xs sm:text-sm text-gray-500 tracking-widest ">
                            {freelancer.email || 'ARCHITECT'}
                        </p>
                        <p className="text-xs sm:text-sm text-gray-500 tracking-widest uppercase">
                            {freelancer.phone|| 'ARCHITECT'}
                        </p>
                        <p className="text-xs sm:text-sm text-gray-500 tracking-widest ">
                            {freelancer.location.city || 'ARCHITECT'}
                        </p>
                        <p className="text-xs sm:text-sm text-gray-500 tracking-widest ">
                            {freelancer.location.country || 'ARCHITECT'}
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        ...
                        <span className="text-gray-600 block sm:inline">
                            {client.name || 'Really Great Company'}
                        </span>
                        <span className="text-gray-600 block ">
                            {client.email || 'Really Great Company'}
                        </span>
                        <span className="text-gray-600 block ">
                            {client.phone || 'Really Great Company'}
                        </span>
                        <span className="text-gray-600 block ">
                            {client.company || 'Really Great Company'}
                        </span>
                    </div>
            ...
            <div className="mb-8">
                {/* Table header */}
                <div className="hidden sm:grid grid-cols-4 gap-4 border-b-2 border-gray-800 pb-2 mb-4 text-xs font-semibold text-gray-800 uppercase tracking-wide">
                    <div>DESCRIPTION</div>
                    <div className="text-center">RATE</div>
                    <div className="text-center">HOURS</div>
                    <div className="text-right">AMOUNT</div>
                ...
                <div className="space-y-3">
                    {works.map((work, idx) => (
                        <div
                            key={idx}
                            className="border border-gray-200 rounded-md p-3 sm:p-0 sm:border-none sm:rounded-none sm:border-b sm:border-gray-200 sm:grid sm:grid-cols-4 sm:gap-4 text-xs sm:text-sm text-gray-600"
                        >
                            ...
                            <div className="text-gray-600 sm:text-center">
                                {work.hours || Math.ceil(work.cost / 50)}
                            </div>
                            <div className="text-gray-800 sm:text-right font-medium">
                                ₹{work.cost.toFixed(2)}
                            </div>
                        </div>
                    ))}
            ...
            <div className="mb-8 text-sm">
                ...
                <div className="text-gray-600 space-y-2">
                    <div className="flex flex-col sm:flex-row sm:justify-between">
                        <span className="font-medium">Bank</span>
                        <span>{freelancer.bank || 'Borcelle Bank'}</span>
                    </div>
                    <div className="flex flex-col sm:flex-row sm:justify-between">
                        <span className="font-medium">Account Name</span>
                        <span>{freelancer.accountName || freelancer.name || 'Samira Hadid'}</span>
                    </div>
                    <div className="flex flex-col sm:flex-row sm:justify-between">
                        <span className="font-medium">Account Number</span>
                        <span>{freelancer.accountNumber || '0000 0000'}</span>
                    </div>
                </div>
            </div>

            {/* Footer contact info */}
            <div className="border-t border-gray-300 pt-4 text-xs sm:text-sm text-gray-500 text-center leading-relaxed">
                <p>
                    {freelancer.location.city  || '123 Anywhere St., Any City'},{freelancer.location.country} —{' '}
                    {freelancer.phone || '123-456-7890'} —{' '}
                    {freelancer.email || 'hello@reallygreatsite.com'}
                </p>
            </div>
